...
# =========================================================
# CAMERA: rear-follow / behind-traffic view
# =========================================================
CAMERA_LOCATION = (0.0, -34.0, 9.0)
CAMERA_TARGET_LOCATION = (0.0, 45.0, 1.5)
CAMERA_LENS_MM = 48

# World / road
ADD_ROAD_PLANE = True
ROAD_PLANE_SIZE = 1500.0
ROAD_Y_CENTER = 160.0
...
# =========================================================
# ORIENTATION / ROAD DIRECTION
# =========================================================
# IMPORTANT:
# Camera is behind traffic, looking forward.
# To see the BACK of vehicles, make them face AWAY from camera.
...
# TRUCK CONTROL
# =========================================================
# To stop random truck spawning, disable trucks completely for now.
ENABLE_TRUCKS = False

# If ENABLE_TRUCKS=False, truck detections become car detections.
MAP_TRUCK_TO_CAR = True

# =========================================================
# EXACT LOCAL FIXES
# =========================================================
EXACT_ASSET_LOCAL_Z_FIX_DEG = {
    ("/home/alien/cv_p3/P3Data/Assets/Vehicles/SedanAndHatchback.blend", "Car"): 180.0,
    ("/home/alien/cv_p3/P3Data/Assets/Vehicles/SUV.blend", "Jeep_3_"): 0.0,
    ("/home/alien/cv_p3/P3Data/Assets/Vehicles/Truck.blend", "Truck"): 0.0,
}
...
def setup_static_scene():
    ...
    bpy.ops.object.light_add(type='AREA', location=(0, -15, 14))
...
def sanitize_asset_class(obj_info):
    asset_class = obj_info.get("asset_class", None)
    if asset_class not in {"car", "truck", "motorcycle", "bicycle", "pedestrian"}:
        return None

    stats = get_bbox_stats(obj_info)
    if stats is None:
        if asset_class == "truck" and not ENABLE_TRUCKS and MAP_TRUCK_TO_CAR:
            return "car"
        return asset_class
    ...
    if asset_class == "truck":
        if not ENABLE_TRUCKS:
            return "car" if MAP_TRUCK_TO_CAR else None

    min_h = MIN_BBOX_HEIGHT_BY_CLASS.get(asset_class, 1.0)
    min_w = MIN_BBOX_WIDTH_BY_CLASS.get(asset_class, 1.0)
...
def dashboard_position_from_detection(obj_info, image_w, image_h):
    ...
    if bbox is not None and len(bbox) == 4:
        ...
        # near object (big bbox / low bottom) => smaller Y
        # far object (small bbox / high in image) => larger Y
        forward_from_height = 2800.0 / (h + 30.0)
        bottom_norm = clamp(bottom / max(image_h, 1.0), 0.0, 1.0)
        forward_from_bottom = FORWARD_MAX - bottom_norm * (FORWARD_MAX - FORWARD_MIN)
        forward = 0.78 * forward_from_height + 0.22 * forward_from_bottom
        forward = clamp(forward, FORWARD_MIN, FORWARD_MAX)

        return Vector((x, forward, GROUND_Z))

    x = float(obj_info.get("x_world", 0.0)) * 5.0
    y = clamp(float(obj_info.get("y_world", 8.0)) * 6.0, FORWARD_MIN, FORWARD_MAX)
    return Vector((x, y, GROUND_Z))

def compute_final_world_yaw(obj_info, asset_class):
    # Motion direction is forward along +Y.
    # Do not flip globally, only use local mesh correction.
...
def main():
    ...
    for local_idx, frame_state in enumerate(all_frames, start=1):
        ...
        for obj_info in objects:
            ...
            asset_class = sanitize_asset_class(obj_info)
